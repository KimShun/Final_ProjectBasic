              ...
              listener: (context, state) {
                switch (state.status) {
                  ...
                  case EventStatus.uploading:
                    context.read<UploadCubit>().uploadImage(state.eventImage!,
                        state.uuid!, "events", "event_banner");
                    break;
...
class _EventImageBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    var bannerImage =
        context.select<EventCubit, File?>((value) => value.state.eventImage);
...
class _EventDetailField extends StatelessWidget {
  ...
  const _EventDetailField({required this.phoneWidth, super.key});
...
class _changeModeToggle extends StatelessWidget {
  ...
  const _changeModeToggle({required this.phoneWidth, super.key});
      ...
      children: [
        Row(
          ...
          children: [
            ...
            Switch(
              value:
                  context.select<EventCubit, bool>((value) => value.state.mode),
              onChanged: (value) {
        ...
        Row(
          ...
          children: [
            ...
            Switch(
              value: context
                  .select<EventCubit, bool>((value) => value.state.imageMode),
              onChanged: (value) {
                context.read<EventCubit>().changeImageMode(value);
              },
              inactiveTrackColor: Colors.white12,
              activeColor: Colors.orange[400],
            ),
...
class _WriteEventBtn extends StatelessWidget {
  ...
  const _WriteEventBtn({required this.phoneWidth, super.key});
      ...
      children: [
        ...
        ElevatedButton(
            onPressed: () {
              context
                  .read<EventCubit>()
                  .changeUuid(const Uuid().v4()); // 게시글 고유번호 생성
              context
                  .read<EventCubit>()
                  .changeDate(DateTime.now()); // 현재 시간으로 등록하기 위함
...
class _EventPostWriteProcess extends StatelessWidget {
  ...
  const _EventPostWriteProcess({required this.phoneWidth, super.key});
